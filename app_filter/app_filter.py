...
def create_rule_precedence_dict(policy_rule_yaml):
    ...
    for key in policy_rule_dict:
        rule_precedence_dict.update(
            {key: policy_rule_dict.get(key).get("precedence")}
        )
        try:
            if policy_rule_dict.get(key).get("pcc-filter-base-name"):
                rule_precedence_dict.update(
                    {
                        policy_rule_dict.get(key)
                        .get("pcc-filter-base-name"): policy_rule_dict.get(key)
                        .get("precedence")
                    }
                )
        except:
...
def create_filter_base_rule_dict(policy_rule_yaml):
    ...
    for key in policy_rule_dict:
        if policy_rule_dict.get(key).get("pcc-filter-base-name") != "null":
            filter_base_rule_dict.update(
                {policy_rule_dict.get(key).get("pcc-filter-base-name"): key}
            )

    return filter_base_rule_dict


def calculate_entry_number(
    application, entries_application_dict, entries_used, rule_precedence_dict
):
    entry_number = (
        int(rule_precedence_dict.get(application))
        if rule_precedence_dict.get(application)
        else 10
    )
    if entry_number not in entries_used:
        ...
        if not entries_application_dict.get(application):
            ...
            entry_number = (
                sorted(entries_application_dict.get(application))[-1] + 5
            )
            entry_number = (
                entry_number
                if entry_number not in entries_used
                else sorted(entries_used)[-1] + 10
            )
            entries_application_dict.get(application).append(entry_number)
        entries_used.add(entry_number)
        return entry_number, entries_application_dict
...
def create_domain_dns_dict(dns_ip_cache_yaml):
    ...
    for dns_ip_cache in dns_ip_cache_dict:
        for rule in dns_ip_cache_dict.get(dns_ip_cache).get("domains"):
            lista = (
                dns_ip_cache_dict.get(dns_ip_cache).get("domains").get(rule)
            )
            for domain in lista:
                if not domain_dns_ip_cache.get(domain):
                    domain_dns_ip_cache.update({domain: dns_ip_cache})
    return domain_dns_ip_cache


def create_app_filter_yaml(
    prefix_list_yaml,
    policy_rule_yaml,
    filter_base_yaml,
    dns_ip_cache_yaml,
        ...
        try:
            if not filter_base_dict.get(key).pop("SPI"):
                ...
                for filter_name in filter_dict:
                    if not (
                        filter_dict.get(filter_name).get("destination-address")
                        or filter_dict.get(filter_name).get(
                            "ipv6-destination-address"
                        )
                        or filter_dict.get(filter_name).get("source-address")
                        or filter_dict.get(filter_name).get(
                            "ipv6-source-address"
                        )
                    ):
                        ip_protocol = filter_dict.get(filter_name).get(
                            "protocol-id"
                        )

                        port = filter_dict.get(filter_name).get(
                        ...
                        protocol = filter_dict.get(filter_name).get(
                            "signature"
                        )
                        domain = filter_dict.get(filter_name).get(
                            "domain-name"
                        )
                        application = key
                        user_agent = filter_dict.get(filter_name).get(
                            "http-user-agent"
                        )
                        (
                            entry_number,
                            entries_application_dict,
                        ) = calculate_entry_number(
                            rule_precedence_dict=rule_precedence_dict,
                            application=application,
                            entries_used=entries_used,
                            entries_application_dict=entries_application_dict,
                        ...
                        app_filter_dict.update(
                            {
                                entry_number: {
                                    ...
                                    "server-port": {
                                        "port": port
                                        if not port_list_dict.get(port)
                                        else None,
                                        "port-list": port_list_dict.get(port),
                                    },
                                    "expression": {
    ...
    for key in policy_rule_filter_dict:
        ...
        for filter_name in policy_rule_filter_dict.get(key):
            ...
            if not (
                filter_dict.get("destination-address")
                or filter_dict.get("ipv6-destination-address")
                or filter_dict.get("source-address")
                or filter_dict.get("ipv6-source-address")
            ):
...
def make_app_filter_unique(app_filter_dict):
    ...
    for entry in app_filter_dict:
        entry_string = "{}".format(
            ...
            + str(
                app_filter_dict.get(entry).get("server-port").get("port-list")
            )
            + str(
                app_filter_dict.get(entry).get("expression").get("http-host")
            )
            + str(app_filter_dict.get(entry).get("expression").get("http-uri"))
            + str(
                app_filter_dict.get(entry)
                .get("server-address")
                .get("ip-prefix-list")
            )
            + str(
        ...
        if entry_string not in entry_set:
            ...
            unique_app_filter_dict.update({entry: app_filter_dict.get(entry)})
...
def create_app_filter_mop(app_filter_yaml, app_filter_commands):
    ...
    provision_commands = (
        read_yaml_file(app_filter_commands).get("commands").get("provision")
    )
    ...
    for entry in app_filter_dict:
        list_of_commands.extend(
            [
                provision_commands.get("create").format(
                    partition="1:1", entry=entry
                ),
                provision_commands.get("description").format(
                    partition="1:1",
                    entry=entry,
                    pr_name=app_filter_dict.get(entry).get("application"),
                ),
                provision_commands.get("application").format(
                    partition="1:1",
                    entry=entry,
        ...
        if (
            ...
            list_of_commands.append(
                provision_commands.get("dns-ip-cache").format(
                    partition="1:1",
                    entry=entry,
                    dns_ip_cache=app_filter_dict.get(entry)
                    .get("server-address")
                    .get("dns-ip-cache"),
                )
            )
            if (
                not app_filter_dict.get(entry)
                .get("expression")
                .get("http-host")
            ):
                if app_filter_dict.get(entry).get("ip-protocol-num") != "17":
                    ...
                    if host:
                        ...
                        if not host.endswith("*"):
                            host = host + "$"
                        list_of_commands.append(
                            provision_commands.get("http-host").format(
                                partition="1:1", entry=entry, http_host=host
                            )
                        )
        elif (
            app_filter_dict.get(entry).get("server-address").get("ip-address")
        ):
            list_of_commands.append(
                provision_commands.get("ip-address").format(
                    partition="1:1",
                    entry=entry,
                    server_address=app_filter_dict.get(entry)
                    .get("server-address")
                    .get("ip-address"),
                )
            )
            ...
            list_of_commands.append(
                provision_commands.get("ip-prefix").format(
                    partition="1:1",
                    entry=entry,
                    prefix_name=app_filter_dict.get(entry)
                    .get("server-address")
                    .get("ip-prefix-list"),
                )
            )
        ...
        if app_filter_dict.get(entry).get("ip-protocol-num") != "0":
            if app_filter_dict.get(entry).get("ip-protocol-num"):
                list_of_commands.append(
                    provision_commands.get("ip_protocol").format(
                        partition="1:1",
                        entry=entry,
        ...
        if app_filter_dict.get(entry).get("server-port").get("port"):
            list_of_commands.append(
                provision_commands.get("server-port").format(
                    partition="1:1",
                    entry=entry,
                    port=app_filter_dict.get(entry)
                    .get("server-port")
                    .get("port"),
                )
            )
        if app_filter_dict.get(entry).get("server-port").get("port-list"):
            list_of_commands.append(
                provision_commands.get("server-port-list").format(
                    partition="1:1",
                    entry=entry,
                    port_list=app_filter_dict.get(entry)
                    .get("server-port")
                    .get("port-list"),
                )
            )
        ...
        if app_filter_dict.get(entry).get("expression").get("http-uri"):
            ...
            if not uri.endswith("*"):
                uri = uri + "$"
            list_of_commands.append(
                provision_commands.get("http-uri").format(
                    partition="1:1", entry=entry, http_uri=uri
                )
            )
        if app_filter_dict.get(entry).get("expression").get("http-user-agent"):
            ...
            if not user_agent.endswith("*"):
                user_agent = user_agent + "$"
            list_of_commands.append(
                provision_commands.get("http-user-agent").format(
                    partition="1:1", entry=entry, user_agent=user_agent
                )
            )
        if app_filter_dict.get(entry).get("protocol"):
            list_of_commands.append(
                provision_commands.get("protocol").format(
                    partition="1:1",
                    entry=entry,
        ...
        list_of_commands.append(
            provision_commands.get("no_shutdown").format(
                partition="1:1", entry=entry
            )
        )
...
def main():
    ...
    create_app_filter_mop(
        path,
        r"C:\Users\ledecast\PycharmProjects\CMG_MoP_Tool\templates\app_filter.yaml",
    )
